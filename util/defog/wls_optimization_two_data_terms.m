function out = wls_optimization_two_data_terms(in1, data_weight1, in2, data_weight2, guidance, lambda)
...
% data_weight1 = data_weight1 - min(data_weight1(:)) ;
% data_weight1 = 1.*data_weight1./(max(data_weight1(:))+small_num);
...
reliability_mask = data_weight1(1,:) < 0.6; % find missing boundary condition
in_row1 = min( in1,[], 1);
data_weight1(1,reliability_mask) = 0.8;
in1(1,reliability_mask) = in_row1(reliability_mask);

Adata1 = spdiags(data_weight1(:), 0, k, k);

if length(data_weight2) == 1
    Adata2 = spdiags(data_weight2*ones(k,1), 0, k, k);
else
    Adata2 = spdiags(data_weight2(:), 0, k, k);
end

A = Adata1 + Adata2 + Asmoothness;
b = Adata1*in1(:) + Adata2*in2(:);
